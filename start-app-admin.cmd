...
:ready
if "%READY%"=="1" (
  powershell -NoProfile -ExecutionPolicy Bypass -Command "$url='%APP_URL%'; $edge='C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe'; $chrome='C:\Program Files\Google\Chrome\Application\chrome.exe'; $extra=@('--disable-pinch','--overscroll-history-navigation=0','--disable-features=TouchpadOverscrollHistoryNavigation,TranslateUI','--no-first-run','--no-default-browser-check','--disable-infobars','--disable-session-crashed-bubble','--disable-background-networking','--disable-component-update'); if (Test-Path $edge) { Start-Process -FilePath $edge -ArgumentList (@('--new-window','--kiosk',$url,'--edge-kiosk-type=fullscreen') + $extra) } elseif (Test-Path $chrome) { Start-Process -FilePath $chrome -ArgumentList (@('--new-window','--kiosk',$url) + $extra) } else { Start-Process $url }"
  exit /b 0
)
